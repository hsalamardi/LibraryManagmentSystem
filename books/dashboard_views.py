...
class DashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Try to get cached dashboard statistics
        ...
        # Get popular books (cache separately as it's more expensive)
        popular_books_key = 'dashboard_popular_books'
        popular_books = cache.get(popular_books_key)
        if popular_books is None:
            popular_books = Book.objects.select_related().annotate(
                borrow_count=Count('borrowings')
            ).filter(borrow_count__gt=0).order_by('-borrow_count')[:10]
            cache.set(popular_books_key, popular_books, 60 * 30)  # Cache for 30 minutes
        
        # Get recently added books (cache for shorter time as it changes more frequently)
        recent_books_key = 'dashboard_recent_books'
        recent_books = cache.get(recent_books_key)
        if recent_books is None:
            recent_books = Book.objects.select_related().order_by('-date_added')[:8]
            cache.set(recent_books_key, recent_books, 60 * 15)  # Cache for 15 minutes
        
        # Get books by language distribution
        language_stats_key = 'dashboard_language_stats'
        language_stats = cache.get(language_stats_key)
        if language_stats is None:
            language_stats = Book.objects.values('language').annotate(
                count=Count('id')
            ).order_by('-count')[:5]
            cache.set(language_stats_key, language_stats, 60 * 60)  # Cache for 1 hour
        
        # Get books by category/genre (cache this too)
        category_stats_key = 'dashboard_category_stats'
        category_stats = cache.get(category_stats_key)
        if category_stats is None:
            category_stats = Book.objects.values('main_class').annotate(
                count=Count('id')
            ).order_by('-count')[:8]
            cache.set(category_stats_key, category_stats, 60 * 60)  # Cache for 1 hour
        ...
        # Enhanced Reading statistics for public interest
        ...
        # Weekly statistics
        books_borrowed_this_week = Borrower.objects.filter(
            borrow_date__gte=last_week
        ).count()
        
        # Monthly statistics
        books_borrowed_this_month = Borrower.objects.filter(
            borrow_date__gte=last_month
        ).count()
        
        new_books_this_month = Book.objects.filter(
            date_added__gte=last_month
        ).count()
        
        new_users_this_month = UserProfileinfo.objects.filter(
            user__date_joined__gte=last_month
        ).count()
        
        # Overdue books
        overdue_books = Borrower.objects.filter(
            return_date__isnull=True,
            due_date__lt=today
        ).count()
        ...
        popular_categories = Book.objects.values('main_class').annotate(
            borrow_count=Count('borrowings')
        ).order_by('-borrow_count')[:5]
        
        # User engagement metrics
        active_users_this_month = Borrower.objects.filter(
            borrow_date__gte=last_month
        ).values('borrower').distinct().count()
        ...
        reading_stats = {
            ...
            'books_borrowed_this_month': books_borrowed_this_month,
            ...
            'average_books_per_user': round(cached_stats['total_books'] / cached_stats['total_users'], 1) if cached_stats['total_users'] > 0 else 0,
            'popular_categories': popular_categories
        }
        
        context.update({
            # Basic Statistics (from cache)
            **cached_stats,
